...
async function reseedHowTo() {
  ...
  const seed = howtoSeed as FaqDocument;
  ...
  try {
    ...
    const topic8 = seed.entries.find((entry) =>
      entry.question.includes("Search Within a Chat Conversation"),
    );
    if (topic8) {
      const content = JSON.parse(topic8.answer) as { steps: { text: string }[] };
      console.log(`Topic 8: ${content.steps.length} steps`);
      content.steps.forEach((step, index) => {
        console.log(`  ${index + 1}. ${step.text}`);
      });
    }

    console.log(`Reseeded ${seed.entries.length} topics into ${seed.id}.`);
  } catch (error) {
